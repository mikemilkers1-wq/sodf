...
export async function requireSession() {
  ...
  const rows = await sql`
    SELECT id, employee_key, display_name, role, status, department, department_head, duty_status
    FROM rcso_employees
    WHERE id = ${session.id}
  `;
...
const ADMIN_COOKIE_NAME = "rcso_admin_access";
...
export async function hasAdminAccess() {
  ...
  try {
    const data = JSON.parse(Buffer.from(payload, "base64url").toString("utf8"));
    return data.scope === "admin" && data.exp > Date.now();
  } catch {
    return false;
  }
}
